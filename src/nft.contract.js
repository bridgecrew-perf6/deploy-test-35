export function nftContract(){
    return `(impl-trait .sip009-nft-trait.nft-trait)
        ...
        (define-data-var max-supply uint u99)
        (define-data-var total-price uint u2000000)
        (define-data-var artiste-address principal 'SP1FWE300K4QMTNDPKV9YDX0NVJ99B138KQ3JN9DT)
        (define-data-var nonce uint u1)
        (define-data-var ipfs (string-ascii 80) "ipfs://ipfs/api-uid/json/")
        ...
        (define-read-only (get-owner (token-id uint))
          (ok (nft-get-owner? NFT-NAME token-id)))
        ...
        ;;     (nft-burn? NFT-NAME token-id tx-sender)))
        
        (define-private (is-owner (token-id uint) (user principal))
            (is-eq user (unwrap! (nft-get-owner? NFT-NAME token-id) false)))
        ...
        (define-private (trnsfr (id uint) (sender principal) (recipient principal))
          (match (nft-transfer? NFT-NAME id sender recipient)
            success
              (let
        ...
        (define-private (is-sender-owner (id uint))
          (let ((owner (unwrap! (nft-get-owner? NFT-NAME id) false)))
        ...
        (define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
          (let ((owner (unwrap! (nft-get-owner? NFT-NAME id) (err err-not-found)))
